...
function SecurityPage() {
  ...
  useEffect(() => {
    if (socket) {
      const handleNewLog = (newLog: any) => {
        if (newLog.status === "failed" || newLog.status === "critical") {
  ...
  const securityAlerts = Array.isArray(logs)
    ? logs.filter((l) => l.status === "failed" || l.status === "critical")
    : [];
  ...
  const SecuritySwitch = ({ label, description, checked, onChange }: any) => (
    <div className="flex items-center justify-between">
      ...
      >
        <div
          className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-all ${checked ? "right-0.5" : "left-0.5"}`}
        ></div>
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          ...
          <p className="text-xl font-bold text-destructive">{securityAlerts.length}</p>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          ...
          <p className="text-sm text-muted-foreground">
            {settings.security_captcha ? "Activé" : "Désactivé"}
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-xl border border-border overflow-hidden flex flex-col">
          <div className="p-5 border-b border-border flex items-center gap-2">
            ...
            <h3 className="font-semibold text-foreground">Alertes de sécurité récentes</h3>
          </div>
          <div className="p-5 flex-1 overflow-auto max-h-[400px]">
            <div className="space-y-4">
              ...
              {securityAlerts.map((log) => (
                <div
                  key={log.id}
                  className={`p-4 rounded-lg border flex flex-col gap-1 ${log.status === "critical" ? "bg-destructive/10 border-destructive/20" : "bg-destructive/5 border-destructive/10"}`}
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`font-semibold text-sm ${log.status === "critical" ? "text-destructive" : "text-foreground"}`}
                    >
                      {log.status === "critical" ? "🚨 " : ""}
                      {log.action}
                    </span>
