...
export const Route = createFileRoute("/admin/payments")({
  head: () => ({ meta: [{ title: "Paiements — COSUMAR" }] }),
...
function PaymentsPage() {
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // 1. Fermer la fenêtre IMMÉDIATEMENT
        setShowModal(false);
        ...
        // 3. Rafraîchir les données un peu après pour ne pas bloquer l'UI
        setTimeout(() => {
          fetchPayments();
        }, 100);
      } else {
        const errorData = await response.json();
        toast.error(`Erreur: ${errorData.message || "Erreur serveur"}`);
      }
    } catch (error) {
    ...
    <>
      <div className="flex items-center justify-between mb-8 animate-fade-in-up">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gestion financière et suivi des factures</p>
        </div>
        <button
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                          payment.status === "Payé" || payment.status === "completed"
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/60 backdrop-blur-sm p-4">
          <div className="bg-card rounded-xl border border-border shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in-up">
            <div className="p-4 border-b border-border flex justify-between items-center bg-muted/20">
              <h3 className="font-bold text-base text-foreground">Nouvelle Transaction</h3>
              <button
            ...
            <form onSubmit={handleAddPayment} className="p-4 space-y-3">
              ...
              <div className="pt-3 flex gap-3">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 px-3 py-2 rounded-lg border border-border font-semibold text-xs hover:bg-muted transition-colors"
                >
                  Annuler
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 bg-primary text-primary-foreground px-3 py-2 rounded-lg font-semibold text-xs hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
